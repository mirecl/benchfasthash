...
var data = []byte("Result.Age.Applicant.External[999]")

func main() {
	fmt.Println(xxh3Hash(data))
	fmt.Println(cityHash(data))
	fmt.Println(t1haHash(data))
}

func xxh3Hash(b []byte) uint64 {
	return xxh3.Hash(b)
}

func cityHash(b []byte) uint64 {
	return cityhash.CityHash64(data)
}

func t1haHash(b []byte) uint64 {
	return Sum64(b, 0)
}
